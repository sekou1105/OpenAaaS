...
/** 跳转到统一认证登录页（页面跳转模式），认证成功后浏览器会被带回 /#/cas/callback */
function goCasLogin() {
  const server = serverStore.servers.find((s) => s.alias === registerAlias.value)
  if (!server) {
    uiStore.addToast('服务器不存在', 'error')
    return
  }
  localStorage.setItem('openaaas_cas_pending_alias', registerAlias.value)
  const base = server.serverUrl.replace(/\/$/, '')
  window.location.href = `${base}/api/v1/client/auth/cas/login`
}
</script>
    ...
    <div class="bg-bg-secondary border border-border rounded-lg p-6 mb-6">
      ...
      <div v-if="showRegisterForm" class="bg-bg-primary border border-border rounded-md p-4 mb-4">
        ...
        <div class="space-y-3">
          <!-- 方式一：CAS 页面跳转（推荐） -->
          <button
            class="w-full px-4 py-2 bg-accent text-white rounded-md text-sm font-medium hover:bg-accent-hover transition-colors"
            @click="goCasLogin"
          >
            统一认证登录（校园网）
          </button>
          <p class="text-xs text-text-muted">跳转到校园统一认证页面登录，密码不经过本系统</p>

          <!-- 方式二：账号密码（RESTful 模式，备用） -->
          <div class="flex items-center gap-2 py-1">
            <div class="flex-1 border-t border-border"></div>
            <span class="text-xs text-text-muted">或使用账号密码</span>
            <div class="flex-1 border-t border-border"></div>
          </div>
          <div>
