...
describe('useServerStore', () => {
  ...
  it('should login by CAS exchange code', async () => {
    const store = useServerStore()
    store.addServer({ alias: 's1', serverUrl: 'http://s1/' })
    mockedHttpFetchWithRedirect.mockResolvedValue({
      ok: true,
      json: async () => ({ api_key: 'ak3', id: 'cid3', name: 'cas-user', role: 'client' }),
    } as Response)

    const result = await store.loginByCas('s1', 'cas_code_abc')
    expect(result.apiKey).toBe('ak3')
    expect(result.clientId).toBe('cid3')
    expect(store.servers[0].apiKey).toBe('ak3')
    expect(store.servers[0].clientName).toBe('cas-user')

    const [url, init] = mockedHttpFetchWithRedirect.mock.calls[0] as [string, RequestInit]
    expect(url).toBe('http://s1/api/v1/client/auth/cas/exchange')
    expect(init.method).toBe('POST')
    expect(JSON.parse(init.body as string)).toEqual({ code: 'cas_code_abc' })
  })

  it('should throw on failed CAS exchange', async () => {
    const store = useServerStore()
    store.addServer({ alias: 's1', serverUrl: 'http://s1/' })
    mockedHttpFetchWithRedirect.mockResolvedValue({
      ok: false,
    ...
    await expect(store.loginByCas('s1', 'bad')).rejects.toThrow('认证失败')
    expect(store.servers[0].apiKey).toBeUndefined()
  })

  it('should throw on CAS exchange for unknown server alias', async () => {
    const store = useServerStore()
    await expect(store.loginByCas('nope', 'code')).rejects.toThrow('服务器不存在')
  })
