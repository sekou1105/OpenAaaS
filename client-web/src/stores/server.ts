...
export const useServerStore = defineStore('server', () => {
  ...
  /**
   * CAS 页面跳转认证：用一次性交换码换取 api_key
   *（浏览器先经 server 跳转到统一认证页，回调后拿到 code）
   */
  async function loginByCas(alias: string, code: string): Promise<{ apiKey: string; clientId: string }> {
    const server = servers.value.find((s) => s.alias === alias)
    if (!server) throw new Error('服务器不存在')

    const url = `${server.serverUrl.replace(/\/$/, '')}/api/v1/client/auth/cas/exchange`
    const res = await httpFetchWithRedirect(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
